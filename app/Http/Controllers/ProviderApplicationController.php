...
class ProviderApplicationController extends Controller
{
    ...
    /**
     * Afficher la page d'explication du rôle prestataire
     */
    public function index()
    {
        ...
        return view('provider-application.index', compact('application'));
    }
    ...
    public function create()
    {
        ...
        if ($application && !$application->canBeEdited()) {
            return redirect()->route('provider-application.status', $application);
        }
    ...
    public function storeStep1(Request $request)
    {
        ...
        // Vérifier si une candidature existe déjà
        $existingApplication = ProviderApplication::where('user_id', Auth::id())->first();
        ...
        if ($existingApplication && !$existingApplication->canBeEdited()) {
            return redirect()->route('provider-application.status', $existingApplication)
        ...
        $application = ProviderApplication::updateOrCreate(
            ['user_id' => Auth::id()],
            [
        ...
        return redirect()->route('provider-application.step2', $application);
    }

    /**
     * Afficher l'étape 2 (spécialisations et formation)
     */
    public function step2(ProviderApplication $application)
    {
        ...
        return view('provider-application.step2', compact('application'));
    }

    /**
     * Sauvegarder l'étape 2
     */
    public function storeStep2(Request $request, ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id()) {
            abort(403);
        }
        ...
        return redirect()->route('provider-application.step3', $application);
    }

    /**
     * Afficher l'étape 3 (documents)
     */
    public function step3(ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id()) {
            abort(403);
        }
        ...
        return view('provider-application.step3', compact('application'));
    }

    /**
     * Sauvegarder l'étape 3 (upload des documents)
     */
    public function storeStep3(Request $request, ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id()) {
            abort(403);
        }
        ...
        try {
            ...
            if ($request->hasFile('motivation_letter')) {
                $letterPath = $this->fileUploadService->upload(
                    ...
                    'provider-applications/motivation-letters',
     ...
     */
    public function status(ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id() && !Auth::user()->isAdmin()) {
            abort(403);
        }

        return view('provider-application.status', compact('application'));
    }

    public function abandon(Request $request, ProviderApplication $application)
    {
        ...
        return redirect()->route('provider-application.create')
     ...
     * Télécharger le CV
     */
    public function downloadCv(ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id() && !Auth::user()->isAdmin()) {
            abort(403);
        }
     ...
     */
    public function downloadMotivationLetter(ProviderApplication $application)
    {
        if ($application->user_id !== Auth::id() && !Auth::user()->isAdmin()) {
            abort(403);
        }
