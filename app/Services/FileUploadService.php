...
class FileUploadService
{
    ...
    protected function getSecureUrl(string $path, ?string $folder = null): string
    {
        ...
        if (! $folder) {
            ...
            if (strpos($cleanPath, 'courses/thumbnails') === 0) {
                ...
                $type = 'community-home';
            } elseif (strpos($cleanPath, 'announcements/') === 0) {
                $type = 'announcements';
            } elseif (strpos($cleanPath, 'richtext/images') === 0) {
                $type = 'richtext-images';
            } else {
    ...
    protected function getTypeFromFolder(string $folder): string
    {
        ...
        } elseif (strpos($folder, 'announcements') !== false) {
            return 'announcements';
        } elseif (strpos($folder, 'richtext/images') !== false) {
    ...
    protected function getBaseDirectoryForType(string $type): ?string
    {
        return match ($type) {
            ...
            'richtext-images' => 'richtext/images',
            default => null,
        };
    }
