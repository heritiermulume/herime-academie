...
@section('admin-subtitle', 'Mettez à jour votre contenu téléchargeable et les informations de fiche.')
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info mb-3">
                            ...
                            <strong>Obligatoire :</strong> conservez le fichier actuel, remplacez-le (glisser-déposer ou parcourir), ou indiquez une URL. Envoi par fragments (chunks) pour les gros fichiers.
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="current-file mb-3 download-current-file" id="downloadCurrentFile">
                                ...
                                <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-2 p-2 bg-light rounded w-100">
                                    ...
                                    <div class="flex-grow-1 text-break text-center text-md-start">
                                        ...
                                        <br>
                                        <small class="text-muted">Fichier déjà en ligne</small>
                                    </div>
                                    <div class="d-flex flex-wrap gap-2 w-100 w-md-auto justify-content-center justify-content-md-end">
                                        <a href="{{ $course->download_file_url }}" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-outline-primary">
                                            <i class="fas fa-external-link-alt me-1"></i>Ouvrir
                                        </a>
                                        <button type="button" class="btn btn-sm btn-outline-danger" id="btnRemoveCurrentDownloadFile" title="Marquer pour suppression (enregistrez ensuite)">
                                            <i class="fas fa-trash me-1"></i>Supprimer
                                        </button>
                                    </div>
                                </div>
                                <p class="small text-muted mt-2 mb-0 d-none" id="downloadFileRemovalNotice">
                                    <i class="fas fa-info-circle me-1"></i>Enregistrez le formulaire après avoir ajouté un nouveau fichier ou une URL — la suppression seule n’est pas suffisante.
                                </p>
                            </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">
                                <i class="fas fa-info-circle me-1"></i>
                                Si le fichier est hébergé ailleurs, indiquez son URL complète (remplace le fichier hébergé sur la plateforme).
                            </small>
                        ...
                        <div class="form-check mb-0">
                            <input class="form-check-input" type="checkbox" id="remove_download_file" name="remove_download_file" value="1" {{ old('remove_download_file') ? 'checked' : '' }}>
                            <label class="form-check-label" for="remove_download_file">
                                Supprimer le fichier actuel (vous devrez alors enregistrer avec un nouveau fichier ou une URL)
                            </label>
                        </div>
                        @endif
                    </div>
...
function handleDownloadFileUpload(input, droppedFile = null) {
    ...
    if (currentFileBlock) {
        currentFileBlock.classList.remove('opacity-75', 'border', 'border-danger', 'rounded');
    }
    if (removalNotice) {
        removalNotice.classList.add('d-none');
    }
    if (btnRemove) {
        btnRemove.disabled = false;
        btnRemove.innerHTML = '<i class="fas fa-trash me-1"></i>Supprimer';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const downloadRemovalNotice = document.getElementById('downloadFileRemovalNotice');
    if (btnRemoveCurrent && removeDownloadCheckbox) {
        btnRemoveCurrent.addEventListener('click', function() {
            removeDownloadCheckbox.checked = true;
            if (currentDownloadBlock) {
                currentDownloadBlock.classList.add('opacity-75', 'border', 'border-danger', 'rounded');
            }
            if (downloadRemovalNotice) {
                downloadRemovalNotice.classList.remove('d-none');
            }
            btnRemoveCurrent.disabled = true;
            btnRemoveCurrent.innerHTML = '<i class="fas fa-check me-1"></i>Marqué pour suppression';
        });
    }
});
