...
@media (max-width: 991.98px) {
    ...
    /* Réduire les paddings des cartes de contenus liés */
    .related-course-card-body {
        padding: 0.5rem;
    }
...
@php
    ...
    try {
        ...
        try {
            ...
            \Log::warning('Erreur lors de l\'accès à video_preview_url', ['content_id' => $course->id, 'error' => $e->getMessage()]);
        }
        ...
        try {
            ...
            \Log::warning('Erreur lors du calcul de hasPreviewLessons', ['content_id' => $course->id, 'error' => $e->getMessage()]);
        }
        ...
        try {
            ...
            \Log::warning('Erreur lors de getWhatYouWillLearnArray', ['content_id' => $course->id, 'error' => $e->getMessage()]);
            $learnings = [];
        }
        ...
        try {
            ...
            \Log::warning('Erreur lors de getRequirementsArray', ['content_id' => $course->id, 'error' => $e->getMessage()]);
            $requirements = [];
        }
        ...
        try {
            ...
            \Log::warning('Erreur lors du calcul de totalLessons', ['content_id' => $course->id, 'error' => $e->getMessage()]);
        }
        ...
        try {
            ...
            \Log::warning('Erreur lors du calcul de totalDuration', ['content_id' => $course->id, 'error' => $e->getMessage()]);
        }
        ...
        try {
            $averageRatingValue = \App\Models\Review::where('content_id', $course->id)
                ->where('is_approved', true)
                ->avg('rating');
            $averageRatingApproved = $averageRatingValue !== null ? round((float)$averageRatingValue, 1) : 0;
            
            $reviewsCountApproved = \App\Models\Review::where('content_id', $course->id)
                ->where('is_approved', true)
                ->count();
            
            $approvedReviews = \App\Models\Review::where('content_id', $course->id)
                ->where('is_approved', true)
                ->with('user')
                ->latest()
                ->get();
        } catch (\Throwable $e) {
            \Log::warning('Erreur lors du chargement des reviews', ['content_id' => $course->id, 'error' => $e->getMessage()]);
        }
    } catch (\Throwable $e) {
            ...
            <div class="col-lg-8">
                        ...
                        <div class="ratio ratio-16x9">
                            @php
                                ...
                                try {
                                    ...
                                    \Log::warning('Erreur lors de l\'accès à thumbnail_url', ['content_id' => $course->id, 'error' => $e->getMessage()]);
                                }
                            @endphp
                            @if($thumbnailUrl)
                ...
                @php
                    ...
                    try {
                        ...
                        \Log::warning('Erreur lors de l\'accès à thumbnail_url (fallback)', ['content_id' => $course->id, 'error' => $e->getMessage()]);
                    }
                @endphp
                ...
                <div class="content-card">
                    ...
                    <div class="instructor-card">
                        @if($course->provider)
                            @php
                                $providerAvatar = '';
                                $providerName = $course->provider->name ?? 'Prestataire';
                                try {
                                    $providerAvatar = $course->provider->avatar_url ?? '';
                                } catch (\Throwable $e) {
                                    \Log::warning('Erreur lors de l\'accès à avatar_url', ['provider_id' => $course->provider->id ?? null, 'error' => $e->getMessage()]);
                                }
                                
                                $providerCoursesCount = 0;
                                $providerStudentsCount = 0;
                                try {
                                    $providerCoursesCount = $course->provider->courses_count ?? ($course->provider->courses ? $course->provider->courses->count() : 0);
                                    
                                    if ($course->provider->courses) {
                                        // Récupérer tous les IDs de cours du prestataire
                                        $providerCourseIds = $course->provider->courses->pluck('id')->toArray();
                                        
                                        // Collecter tous les user_id uniques depuis différentes sources
                                        $uniqueUserIds = collect();
                                        
                                        // 1. Utilisateurs ayant téléchargé au moins un contenu téléchargeable
                                        $downloadUserIds = \App\Models\CourseDownload::whereIn('content_id', $providerCourseIds)
                                            ->whereNotNull('user_id')
                                            ->distinct()
                                            ->pluck('user_id');
                                        $uniqueUserIds = $uniqueUserIds->merge($downloadUserIds);
                                        
                                        // 2. Utilisateurs ayant acheté au moins un contenu (via OrderItem avec commande payée/complétée)
                                        $purchaseUserIds = \App\Models\Order::whereIn('id', function($query) use ($providerCourseIds) {
                                                $query->select('order_id')
                                                    ->from('order_items')
                                                    ->whereIn('content_id', $providerCourseIds);
                                            })
                                            ->whereIn('status', ['paid', 'completed'])
                                            ->whereNotNull('user_id')
                                            ->distinct()
                                            ->pluck('user_id');
                                        $uniqueUserIds = $uniqueUserIds->merge($purchaseUserIds);
                                        
                                        // 3. Utilisateurs inscrits aux contenus non téléchargeables
                                        $enrollmentUserIds = \App\Models\Enrollment::whereIn('content_id', $providerCourseIds)
                                            ->whereNotNull('user_id')
                                            ->distinct()
                                            ->pluck('user_id');
                                        $uniqueUserIds = $uniqueUserIds->merge($enrollmentUserIds);
                                        
                                        // Compter les utilisateurs uniques
                                        $providerStudentsCount = $uniqueUserIds->unique()->count();
                                    }
                                } catch (\Throwable $e) {
                                    \Log::warning('Erreur lors du calcul des stats instructeur', ['provider_id' => $course->provider->id ?? null, 'error' => $e->getMessage()]);
                                }
                            @endphp
                        ...
                        <div class="flex-grow-1">
                            <h5>{{ $course->provider->name ?? 'Prestataire' }}</h5>
                            @if($course->provider && $course->provider->bio)
                            <p class="text-muted mb-0">{{ Str::limit($course->provider->bio, 200) }}</p>
                            @endif
                            <div class="instructor-stats">
                                <span><i class="fas fa-book me-1"></i>{{ $providerCoursesCount ?? 0 }} contenus</span>
                                <span><i class="fas fa-users me-1"></i>{{ $providerStudentsCount ?? 0 }} {{ ($providerStudentsCount ?? 0) > 1 ? 'participants' : 'participant' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="content-card">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="{{ route('contents.reviews', $course->slug) }}" class="btn btn-outline-primary" style="font-size: 0.75rem; padding: 0.25rem 0.625rem; line-height: 1.4;">
                                        ...
                                        @else
                                            ...
                                            @if($course->is_sale_enabled ?? true)
                                                <form action="{{ route('customer.contents.enroll', $course->slug) }}" method="POST">
                                                    ...
                                                    <button type="submit" class="btn btn-primary btn-lg w-100">
                                                        <i class="fas fa-user-plus me-2"></i>S'inscrire au contenu
                                                    </button>
                                                </form>
                                            @else
                                ...
                                @else
                                    ...
                                    @elseif($hasPurchased)
                                        ...
                                        @if($course->is_downloadable)
                                            ...
                                            @endif
                                        @else
                                            {{-- Pour les contenus non téléchargeables, proposer l'inscription --}}
                                            @if($course->is_sale_enabled ?? true)
                                                <form action="{{ route('customer.contents.enroll', $course->slug) }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="redirect_to" value="learn">
                        ...
                        <ul class="course-features-list">
                            @php
                                ...
                                try {
                                    ...
                                    \Log::warning('Erreur lors de getCourseFeatures', ['content_id' => $course->id, 'error' => $e->getMessage()]);
                                    // Valeurs par défaut
                                    $courseFeatures = [
        ...
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header text-white border-0 flex-shrink-0" style="background-color: #003366;">
                <h5 class="modal-title fw-bold" id="coursePreviewModalLabel">
                    <i class="fas fa-play-circle me-2"></i>Aperçus du contenu
                </h5>
                    ...
                    <div class="col-lg-8" style="border-right: 1px solid #dee2e6;">
                        <div class="p-4">
                            ...
                            <div class="bg-light rounded p-3" id="previewLessonInfo" style="margin-top: 0.25rem !important; padding-top: 0.5rem !important; padding-bottom: 0.75rem !important;">
                                <h6 class="fw-bold mb-2" style="color: #003366; margin-top: 0;">Aperçu du contenu</h6>
...
<script>
    window.coursePreviewData = {
        courseId: {{ $course->id }},
        previewUrl: '{{ route('contents.preview-data', $course->slug) }}'
    };
</script>
@endif
