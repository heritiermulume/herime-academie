...
@section('admin-title', 'Gestion des Prestataires')
@section('admin-subtitle', 'Consultez et gérez les prestataires, leurs candidatures et leurs paiements')
        ...
        <div class="admin-panel__body">
            ...
            <ul class="nav nav-tabs mb-4" id="providersTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ ($tab ?? 'providers') === 'providers' ? 'active' : '' }}" 
                            id="providers-tab" 
                            data-bs-toggle="tab" 
                            data-bs-target="#providers" 
                            type="button" 
                            role="tab"
                            onclick="window.location.href='{{ route('admin.provider-applications', ['tab' => 'providers']) }}'">
                        <i class="fas fa-chalkboard-teacher me-2"></i>Prestataires
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ ($tab ?? 'providers') === 'payouts' ? 'active' : '' }}" 
                            id="payouts-tab" 
                            data-bs-toggle="tab" 
                            data-bs-target="#payouts" 
                            type="button" 
                            role="tab"
                            onclick="window.location.href='{{ route('admin.provider-applications', ['tab' => 'payouts']) }}'">
                        <i class="fas fa-money-bill-wave me-2"></i>Paiements
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ ($tab ?? 'providers') === 'applications' ? 'active' : '' }}" 
                            id="applications-tab" 
                            data-bs-toggle="tab" 
                            data-bs-target="#applications" 
                            type="button" 
                            role="tab"
                            onclick="window.location.href='{{ route('admin.provider-applications', ['tab' => 'applications']) }}'">
            ...
            <div class="tab-content" id="providersTabContent">
                ...
                <div class="tab-pane fade show active" 
                    ...
                    <div class="admin-stats-grid mb-4">
                        <div class="admin-stat-card">
                            ...
                            <p class="admin-stat-card__muted">Prestataires</p>
                        </div>
                        <div class="admin-stat-card">
                    ...
                    <x-admin.search-panel
                        :action="route('admin.provider-applications', ['tab' => 'providers'])"
                        formId="providersFilterForm"
                        filtersId="providersFilters"
                        :hasFilters="true"
                    ...
                    >
                        <x-slot:filters>
                            ...
                            <div class="d-flex justify-content-between align-items-center gap-2">
                                <span class="text-muted small">Filtrez rapidement les prestataires selon leur statut.</span>
                                <a href="{{ route('admin.provider-applications', ['tab' => 'providers']) }}" class="btn btn-outline-secondary">
                                    ...
                                    @forelse($applications ?? [] as $application)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if(isset($application->is_virtual) && $application->is_virtual)
                                                    <a href="{{ route('admin.users.show', $application->user) }}"
                                                       class="btn btn-light btn-sm" title="Voir le prestataire">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                @else
                                                    <a href="{{ route('admin.provider-applications.show', $application) }}"
                ...
                <div class="tab-pane fade show active" 
                    ...
                    <x-admin.search-panel
                        :action="route('admin.provider-applications', ['tab' => 'payouts'])"
                    ...
                    >
                        <x-slot:filters>
                            ...
                            <div class="d-flex justify-content-between align-items-center gap-2">
                                ...
                                <a href="{{ route('admin.provider-applications', ['tab' => 'payouts']) }}" class="btn btn-outline-secondary">
                                    <i class="fas fa-undo me-2"></i>Réinitialiser
                                </a>
                            </div>
                        </x-slot:filters>
                    ...
                    <!-- Filtres actifs -->
                    @if(request('search') || request('status') || request('provider_id'))
                    <div class="alert alert-info d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                        <div>
                            ...
                            @if(request('provider_id'))
                                <span class="badge bg-warning ms-2">Prestataire: {{ ($providers ?? collect())->firstWhere('id', request('provider_id'))->name ?? 'N/A' }}</span>
                            @endif
                        </div>
                        <a href="{{ route('admin.provider-applications', ['tab' => 'payouts']) }}" class="btn btn-sm btn-outline-danger">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <img src="{{ $payout->provider->avatar_url }}" alt="{{ $payout->provider->name }}" 
                                                     class="admin-user-avatar" style="width: 32px; height: 32px;">
                                                <div>
                                                    <div class="fw-semibold">{{ $payout->provider->name }}</div>
                                                    <small class="text-muted">{{ $payout->provider->email }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.contents.show', $payout->course) }}" class="text-decoration-none">
                ...
                <div class="tab-pane fade show active" 
                    ...
                    <x-admin.search-panel
                        :action="route('admin.provider-applications', ['tab' => 'applications'])"
                    ...
                    >
                        <x-slot:filters>
                            ...
                            <div class="d-flex justify-content-between align-items-center gap-2">
                                ...
                                <a href="{{ route('admin.provider-applications', ['tab' => 'applications']) }}" class="btn btn-outline-secondary">
                                    <i class="fas fa-undo me-2"></i>Réinitialiser
                                </a>
                            </div>
                        </x-slot:filters>
