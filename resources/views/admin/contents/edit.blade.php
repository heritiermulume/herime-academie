...
@section('admin-title', 'Modifier un contenu')
@section('admin-subtitle', "Actualisez l'ensemble des informations du contenu « " . Str::limit(html_entity_decode($course->title, ENT_QUOTES, 'UTF-8'), 60) . " »")
@section('admin-actions')
    <a href="{{ route('admin.contents') }}" class="btn btn-light" data-temp-upload-cancel>
...
@section('admin-content')
    ...
    <form action="{{ route('admin.contents.update', $course) }}" method="POST" id="courseForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <label for="provider_id" class="form-label fw-bold">Prestataire <span class="text-danger">*</span></label>
                                <select class="form-select @error('provider_id') is-invalid @enderror" 
                                        id="provider_id" name="provider_id" required>
                                    <option value="">Sélectionner un prestataire</option>
                                    @foreach($providers as $provider)
                                        <option value="{{ $provider->id }}" {{ old('provider_id', $course->provider_id) == $provider->id ? 'selected' : '' }}>
                                            {{ $provider->name }} 
                                            @if($provider->role === 'admin' || $provider->role === 'super_user')
                                                ({{ $provider->role === 'super_user' ? 'Super Admin' : 'Admin' }})
                                            @else
                                                (Prestataire)
                                            @endif
                                        </option>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('short_description') is-invalid @enderror" 
                                          ...
                                          rows="3"
                                          placeholder="Une introduction concise affichée dans la liste des contenus">{{ old('short_description', $course->short_description) }}</textarea>
                                <small class="form-text text-muted">Une introduction courte qui sera visible dans les listes de contenus</small>
                            ...
                            <div class="col-12">
                                <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center justify-content-between gap-2 mb-1">
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-primary insert-embed-btn embed-insert-btn" data-target-textarea-id="description">
                                        <i class="fas fa-link me-1"></i>Insérer un lien embarqué
                                    </button>
                                </div>
                                <textarea class="form-control rich-text-editor @error('description') is-invalid @enderror" 
                                          id="description" 
                                          name="description" 
                                          rows="5"
                                          data-editor-placeholder="Décrivez le contenu et les objectifs de la formation..."
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <div class="form-check mt-4">
                                    <input class="form-check-input" type="checkbox" id="use_fake_promo_countdown" name="use_fake_promo_countdown" value="1"
                                           ...
                                           {{ old('use_fake_promo_countdown', $course->use_fake_promo_countdown ?? false) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-2" id="fakePromoDurationWrapper" style="display: {{ old('use_fake_promo_countdown', $course->use_fake_promo_countdown ?? false) ? 'block' : 'none' }};">
                                ...
                                <input type="number" class="form-control @error('fake_promo_duration_days') is-invalid @enderror"
                                       ...
                                       value="{{ old('fake_promo_duration_days', $course->fake_promo_duration_days ?? 3) }}" min="1" max="365">
                            ...
                            <div class="col-12">
                                ...
                                <div id="required_subscription_tier_group" class="mt-2" style="display: {{ old('requires_subscription', $course->requires_subscription) ? 'block' : 'none' }};">
                                    ...
                                    <select class="form-select @error('required_subscription_tier') is-invalid @enderror" id="required_subscription_tier" name="required_subscription_tier">
                                        <option value="quarterly" {{ old('required_subscription_tier', $course->normalizedRequiredMemberPeriod()) === 'quarterly' ? 'selected' : '' }}>Trimestriel</option>
                                        <option value="semiannual" {{ old('required_subscription_tier', $course->normalizedRequiredMemberPeriod()) === 'semiannual' ? 'selected' : '' }}>Semestriel</option>
                                        <option value="yearly" {{ old('required_subscription_tier', $course->normalizedRequiredMemberPeriod()) === 'yearly' ? 'selected' : '' }}>Annuel</option>
                                        <option value="all" {{ old('required_subscription_tier', $course->normalizedRequiredMemberPeriod()) === 'all' ? 'selected' : '' }}>Toutes périodes (Membre)</option>
                                    </select>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="show_customers_count" name="show_customers_count" value="1" 
                                           {{ old('show_customers_count', $course->show_customers_count) ? 'checked' : '' }}>
                            ...
                            <div class="col-12">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="send_receipt_enabled" name="send_receipt_enabled" value="1"
                                           {{ old('send_receipt_enabled', $course->send_receipt_enabled ?? false) ? 'checked' : '' }}
                        ...
                        <div class="row mt-3">
                            <div class="col-12" id="receipt-custom-fields" style="display: {{ old('send_receipt_enabled', $course->send_receipt_enabled ?? false) ? 'block' : 'none' }};">
                                <div class="alert alert-info mb-3">
                                <i class="fas fa-info-circle me-1"></i>
                            ...
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control @error('receipt_custom_title') is-invalid @enderror"
                                           ...
                                           value="{{ old('receipt_custom_title', $course->receipt_custom_title) }}"
                                ...
                                <div class="col-12 mb-4">
                                    ...
                                    <textarea class="form-control font-monospace @error('receipt_custom_body') is-invalid @enderror"
                                              ...
                                              placeholder="Bonjour {user_name}, ...">{{ old('receipt_custom_body', $course->receipt_custom_body) }}</textarea>
                ...
                <div class="card shadow-sm mb-4 form-actions-card">
                    <div class="card-body">
                        <div class="form-actions d-flex flex-column flex-sm-row gap-2 align-items-stretch align-items-sm-center">
                            <a href="{{ route('admin.contents') }}" class="btn btn-secondary form-actions__btn" data-temp-upload-cancel>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ne pas ajouter de section vide si le contenu n'en a pas : l'utilisateur peut cliquer sur "Ajouter une section"
});
...
// Gestion des champs du reçu PDF
...
(function() {
    ...
    function ensureModernDialogModal() {
        let modalEl = document.getElementById('richtextModernDialog');
        if (modalEl) {
            return modalEl;
        }
        ...
        modalEl.id = 'richtextModernDialog';
        modalEl.tabIndex = -1;
        modalEl.innerHTML = `
    ...
    function uploadRichEditorImageInChunks(file, onProgress) {
        return new Promise((resolve, reject) => {
            const inlineUploadId = `rich-inline-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
            const uploadTaskId = createUploadTask(
                file?.name || 'Image description',
                file?.size || 0,
                'Téléversement de l’image de la description'
            );
    ...
    function hasPendingOrLocalImages() {
        for (const [, quill] of quillInstances) {
    ...
    function initQuillEditor(textarea) {
        ...
        container.style.height = textarea.classList.contains('rich-text-editor') ? '260px' : '300px';
        ...
        const quill = new Quill(container, {
            ...
            placeholder: textarea.dataset.editorPlaceholder || 'Saisissez le contenu de la leçon...'
        });
    ...
    function initAllEditors() {
        const textareas = document.querySelectorAll('.lesson-content-text-editor, .rich-text-editor');
        ...
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // Element node
                    const textareas = node.querySelectorAll ? node.querySelectorAll('.lesson-content-text-editor, .rich-text-editor') : [];
                    textareas.forEach(function(textarea) {
                        initQuillEditor(textarea);
                    });
                    // Si le node lui-même est un textarea
                    if (node.classList && (node.classList.contains('lesson-content-text-editor') || node.classList.contains('rich-text-editor'))) {
    ...
    document.addEventListener('click', async function (event) {
        const button = event.target.closest('.insert-embed-btn');
        if (!button) {
            return;
        }
        ...
        const quill = quillInstances.get(textarea);
