...
<head>
    ...
    <title>Paiement reçu - {{ config('app.name') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="payout-details">
                ...
                @if($payout->order)
                <p><strong>Numéro de commande :</strong> {{ $payout->order->order_number }}</p>
                @endif
                
                @if($payout->commission_amount > 0)
                <p><strong>Commission déduite :</strong> {{ number_format($payout->commission_amount, 2) }} {{ $payout->currency }} ({{ number_format($payout->commission_percentage, 2) }}%)</p>
                @endif
                
                @if($processedAtText)
                <p><strong>Date de traitement :</strong> {{ $processedAtText }}</p>
                @endif
                
                @if($payout->provider_transaction_id)
                <p><strong>Référence transaction :</strong> {{ $payout->provider_transaction_id }}</p>
                @endif
            </div>
