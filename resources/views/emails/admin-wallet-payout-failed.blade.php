...
<head>
    ...
    <title>Payout wallet échoué - {{ config('app.name') }}</title>
    <style>
        ...
        .header { border-bottom: 3px solid #dc3545; padding-bottom: 20px; margin-bottom: 24px; }
        .header h1 { color: #dc3545; font-size: 22px; margin: 0 0 8px 0; }
        .muted { color: #6c757d; font-size: 13px; margin: 0; }
        .box { background: #f8f9fa; border-left: 4px solid #dc3545; padding: 16px; border-radius: 4px; margin: 16px 0; }
        .box p { margin: 6px 0; }
        .reason { background: #fff5f5; padding: 12px; border-radius: 4px; margin: 12px 0; font-weight: 500; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            @endif
            <h1>Payout wallet échoué</h1>
            <p class="muted">{{ config('app.name') }} - Notification admin</p>
        </div>

        <p>{{ $timeGreeting }} {{ $adminName }},</p>
        <p>Un retrait (payout) depuis le portefeuille admin a échoué. Le montant a été ou sera remboursé sur le portefeuille concerné.</p>
        ...
        <div class="box">
            ...
            @else
                <p><strong>Compte destination (tentative) :</strong></p>
                <p>— Téléphone : {{ $payout->phone ?? '—' }}</p>
                <p>— Opérateur / Méthode : {{ $payout->method ?? '—' }}</p>
                <p>— Pays : {{ $payout->country ?? '—' }}</p>
            @endif
            @if($payout->wallet && $payout->wallet->user)
                <p><strong>Portefeuille concerné (titulaire) :</strong> {{ $payout->wallet->user->name ?? '—' }} ({{ $payout->wallet->user->email ?? '—' }})</p>
            @endif
            @if($payout->failed_at)
                <p><strong>Date de l'échec :</strong> {{ $payout->failed_at->timezone(config('app.timezone'))->format('d/m/Y à H:i') }}</p>
            @endif
        </div>
